  ...
  return (
    <>
      <div className="flex flex-col gap-5">
        ...
        <div>
          <label
            htmlFor="selectCounty"
            className={`block font-medium ${pinkAsterisk}`}
          >
            Judet
          </label>
          <select
        ...
        {validationError && (
          <span className="text-red-500">Campurile nu sunt completate</span>
        )}
      </div>
...
export default function BusinessForm({
  ...
  return (
    <div>
      <div className={`text-pink-600 text-sm font-medium mb-4 ${pinkAsterisk}`}>
        Campuri necesare
      </div>
      <form onSubmit={handleSubmit} className="space-y-4 md:w-1/2 max-w-100">
        <div>
          <label className={`block font-medium ${pinkAsterisk}`}>
            Numele afacerii
          </label>
          <input
            name="name"
            ...
            {categories.map((group) => (
              ...
              >
                {group.items.map((item) => (
                  <option className="text-red-100" key={item}>
                    {item}
                  </option>
                ))}
              </optgroup>
        ...
        <div className="h-50 max-w-100 md:w-80 border border-dashed border-gray-300 rounded-md flex flex-col justify-center items-center">
          <label
            htmlFor="photo"
            className={`block font-medium  border rounded border-gray-300 w-30 text-center py-1 cursor-pointer`}
          >
            <div className="relative">
              <Image
                src="/upload.svg"
          ...
          <input
            ...
            className="w-1/3 border rounded border-gray-300 hidden"
          />
          <div className="w-50 h-20 text-center flex justify-center">
            ...
            {preview && (
              <Image
                ...
                width={100}
                height={100}
                className="mt-2 rounded w-60 h-20 object-cover"
              />
            )}
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          <label htmlFor="isOnline">Este afacere online</label>
          <input
            type="checkbox"
            id="isOnline"
            name="isOnline"
        ...
        <Button
          ...
          text={
            initialData?.name
              ? "Modificati locatiile fizice"
              : "Adaugati locatii fizice"
          }
        />
        {showAddressesForm && (
