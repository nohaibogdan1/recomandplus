...
export default function BusinessFilters({ params }: { params: Params }) {
    ...
    return (
        <div className="relative">
            ...
            {showFilter && <Filters params={params} close={() => setShowFilter(false)} />}
        </div>
    )
}
...
function Filters({ close, params }: { close: () => void, params: Params }) {
    ...
    return (
        <div ref={modalRef} className={`transition-all duration-300 absolute bg-white h-100 w-70 px-3 py-6 pb-1 rounded-md flex flex-col gap-3 opacity-100 scale-100 translate-y-0`}>
            <div className="flex flex-col space-y-3 h-5/6">
                <Toggle checked={online} handleChange={() => setOnline(!online)} label="Online" />
